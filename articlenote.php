	...
	<head>
		...
		<script>	
			...
			<?php
				...
				$like = 0;//讚數
				$post_ID = $_GET['post_ID'];
				$data2 = "SELECT COUNT(*) as total FROM like_post where post_ID = '$post_ID' ";
				...
				//確認此user是否按過讚
				$data2 = "SELECT * FROM like_post where post_ID = '$post_ID' ";
				if($stmt = mysqli_query($db,$data2)){
				...
				$picture=array();
					$i=0;
					$j=0;
					$data3 = "SELECT * FROM noteimg where post_ID='$post_ID'";
			...
			$(document).ready(function()
			{
				$("#like").click(function()
				{
					if(bool == 0)
					{
						$.post('add_like_post.php', {post_ID: post_ID});
						bool = 1;
						CheckLikeNumber();
					}
					else
					{
						$.post('delete_like_post.php', {post_ID: post_ID});
						bool = 0;
						CheckLikeNumber();
					}
				});
...
<body onload="viewpicter()">
	<?php 
		$id=$_GET['post_ID'];
		$sql = "SELECT * FROM uploadnote where post_ID='$id'";
    ...
    <div class="col-9 buttonregion">
		...
		<button class="button" style="vertical-align:middle" data-toggle="modal" data-target="#reportnote"><span>檢舉 </span></button>
	</div>
  </div>
  </center>
  <div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="reportnote">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content answerblock">
					<br>
					<form method="post" action="add_report_note.php?post_ID=<?php echo $id;?>" class="ccform">
