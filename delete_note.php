...
if($_SESSION['user_ID'] != null)
{
	...
	if (!isset($_GET["page"])){ //假如$_GET["page"]未設置
		...
		{
			$psize=5; //每頁顯示五筆資料
			$sql2 = "select count(*) as total from uploadnote where user_ID='$id'";//尋找此帳號的所有帖子
			...
			if($result2 > 0){
				...
				$offset = ($page-1)*$psize;
				$sql3 = "select * from uploadnote where user_ID='$id' order by post_time desc limit $offset,$psize";
...
<?php
				...
				{
					$title=$row['Title'];
					$time=$row['post_time'];             //*不知道下一行是否要設帖子的超連結????*
					$noteID=$row['post_ID'];
?>
<tr>
	...
	<td><?php echo $time; ?></td>
	<td align="center">
		<form action="delete_note_finish.php?nID=<?php echo $noteID;?>" method="post" name="form1">
				<button class="button2" style="vertical-align:middle"><span>刪除貼文 </span></button>
		</form>
		<form action="articlenote.php?post_ID=<?php echo $row['post_ID'];?>" method="post" name="form2">
