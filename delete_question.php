...
if($_SESSION['user_ID'] != null)
{
	...
	if (!isset($_GET["page"])){ //假如$_GET["page"]未設置
		...
		{
			$psize=5; //每頁顯示五筆資料
			$sql2 = "select count(*) as total from question where user_ID='$id'";//尋找此帳號的所有帖子
			$result2 = mysql_query($sql2);
			if($result2 >0){
				$dcount = mysql_result($result2,0);       //總資料數
				$pcount = ceil($dcount/$psize);                 //總頁數
				$offset = ($page-1)*$psize;
				$sql3 = "select * from question where user_ID='$id' order by question_time desc limit $offset,$psize";
...
<?php
				while ($row = mysql_fetch_array($result3))           //輸出資料內容
				{
					...
					$time=$row['question_time'];             //*不知道下一行是否要設帖子的超連結????*
					$qid=$row['question_ID'];
?>
<tr>
    <td><?php echo mb_strimwidth($title,0,15,"...","UTF-8"); ?></td> 
	<td><?php echo $time; ?></td>
	<td>
		<form action="delete_question_finish.php?qtime=<?php echo $time;?>&QID=<?php echo $qid;?>" method="post" name="form1">
...
<br />
<?php
				echo '記錄:共 '.$dcount.' 筆 頁次: '.$page.' / '.$pcount.' 頁'; //分頁操作
				...
				echo " </strong>頁 <a href=?page=".$pcount.">末頁</a><br /><br />";		
				}
				else{
					echo'查無貼文';
			}
